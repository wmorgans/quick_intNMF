...
class intNMF():
    ...
    def fit(self, rna_mat, atac_mat):
        ...
        Uses accelerated Hierarchical alternating least squares algorithm
        proposed here, but modified to joint factorise two matrices.
        https://arxiv.org/pdf/1107.5194.pdf. Only required arguments are the
        matrices to use for factorisation. GEX and ATAC matrices are expected
        in cell by feature format. Matrices should be scipy sparse matrices.
        min ||X_rna - (theta . phi_rna)||_2 and
        min ||X_atac - (theta . phi_atac)||_2 s.t. theta, phi_rna, phi_atac > 0.
        So that theta hold the latent topic scores for a cell. And phi
    ...
    def _HALS(self, H, WtW, WtM, eit1, alpha=0.5, delta=0.1):
        """
        Optimizing min_{V >= 0} ||M-UV||_F^2.

        With an exact block-coordinate descent schemeUpdate V.
        ...
        AND
        sum of squares of updates to V at current epoch > 0.01 * sum of squares
        of updates to V at first epoch
        """
        r, n = H.shape
