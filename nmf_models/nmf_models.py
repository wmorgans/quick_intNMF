...
class intNMF():
    ...
    def __init__(self, n_topics, epochs=200, init = 'random'):
        """
        Parameters
        ----------
    ...
    def fit(self, rna_mat, atac_mat):
        ...
        for i in range(self.epochs):
            ...
            A = safe_sparse_dot(RNA_mat, phi_rna.T)
            B = phi_rna.dot(phi_rna.T)
            eit1 = time.perf_counter() - eit1
           
            if i == 0:
                scale = np.sum(A*theta)/np.sum(B*(theta.T.dot(theta)))
                theta=theta*scale

            
            theta, theta_rna_it = self._HALS(theta.T, B.T, A.T, eit1)
            theta = theta.T
            ...
            #update theta
            eit1 =time.perf_counter()
            A = safe_sparse_dot(ATAC_mat, phi_atac.T)
            B = phi_atac.dot(phi_atac.T)
            eit1 = time.perf_counter() - eit1
            
            theta, theta_atac_it = self._HALS(theta.T, B.T, A.T, eit1)
            theta = theta.T
            ...
            logging.info('theta rna iter: {}\nphi rna iter: {}\ntheta atac iter: {}\nphi atac iter: {}\n'.format(theta_rna_it, phi_rna_it, theta_atac_it, phi_atac_it))
            self.epoch_times.append(epoch_duration)
            self.loss.append(error_rna+error_atac)
            self.loss_atac.append(error_atac)
            self.loss_rna.append(error_rna)
            self.epoch_iter.append(theta_rna_it + phi_rna_it + theta_atac_it + phi_atac_it)
    ...
    def _initialize_nmf(self, X, ATAC_mat, n_components, eps=1e-6, random_state=None, init='random'):
